...
import { Table, Tag, Space, Typography, Card, Button, Input, message, Avatar } from 'antd';
  ...
  const columns = [
    {
      ...
      render: (text: string, record: User) => (
        <Space size="middle">
          ...
          <div>
            <Text strong style={{ display: 'block', color: '#1e293b' }}>{text}</Text>
            <Text type="secondary" style={{ fontSize: 12 }}>ID: {record._id?.slice(-6).toUpperCase() || 'N/A'}</Text>
          </div>
        </Space>
      ),
    },
    ...
    },
    {
      ...
      render: (active: boolean) => (
        <Space size="small" style={{ color: active ? '#10b981' : '#ef4444', fontWeight: 600 }}>
    ...
    },
    {
      ...
      render: (_: any, record: User) => (
        <Space size="small">
          <Button type="text" size="small" style={{ color: '#6366f1' }}>Edit</Button>
    ...
    <DashboardLayout>
      <div className="page-header">
        <div>
          ...
          <Text className="page-subtitle">Manage system users, roles, and access permissions</Text>
        </div>
      ...
      <Card variant="borderless">
        <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', gap: 16 }}>
          <Space size="middle">
            ...
            />
            <Button icon={<MdRefresh size={20} />} onClick={() => refetch()} loading={isLoading}>
              Refresh
            </Button>
          </Space>
        </div>
        ...
        <Table
          ...
          pagination={{
            ...
            itemRender: (page, type, originalElement) => {
              if (type === 'prev') return <Text style={{ color: '#6366f1', fontWeight: 600 }}>Previous</Text>;
              if (type === 'next') return <Text style={{ color: '#6366f1', fontWeight: 600 }}>Next</Text>;
